/*  Adapted from float3.h in NViSII repository by Nathan Morrical and Jonathan Tremblay 
    and Yunzhi Lin and Stephen Tyree and Stan Birchfield and Valerio Pascucci
    and Ingo Wald
...
{
    glm::mat4 xfm;
    xfm = glm::column(xfm, 0, glm::vec4(xfm_[0], xfm_[4],  xfm_[8], 0.0f));
    xfm = glm::column(xfm, 1, glm::vec4(xfm_[1], xfm_[5],  xfm_[9], 0.0f));
    xfm = glm::column(xfm, 2, glm::vec4(xfm_[2], xfm_[6],  xfm_[10], 0.0f));
    xfm = glm::column(xfm, 3, glm::vec4(xfm_[3], xfm_[7],  xfm_[11], 1.0f));
	return xfm;
}
...
__device__ float4 operator*(const glm::mat4 &l, const float4 &r) {
